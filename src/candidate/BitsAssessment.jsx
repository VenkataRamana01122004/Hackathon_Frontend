...
const MAX_RESUMES = 1;  
const DISPLAY_GRACE_PERIOD = 60;
const DISPLAY_CHECK_INTERVAL = 5000;
...
export default function BitsAssessment() {
  ...
  const [isDisplayViolated, setIsDisplayViolated] = useState(false);
  const [displayGraceRemaining, setDisplayGraceRemaining] = useState(DISPLAY_GRACE_PERIOD);
  const [displayViolationMessage, setDisplayViolationMessage] = useState("");
  ...
  const isDisplayViolatedRef = useRef(isDisplayViolated);
  const displayViolationMessageRef = useRef(displayViolationMessage);
  ...
  useEffect(() => { isDisplayViolatedRef.current = isDisplayViolated; }, [isDisplayViolated]);
  useEffect(() => { displayViolationMessageRef.current = displayViolationMessage; }, [displayViolationMessage]);
...
window.addEventListener("keydown", restoreFullscreen);
  ...
  useEffect(() => {
    ...
    const runDisplayCheck = async () => {
      if (cancelled) return;
      await evaluateDisplaySetup();
    };

    runDisplayCheck();
    const interval = setInterval(runDisplayCheck, DISPLAY_CHECK_INTERVAL);

    window.addEventListener("focus", runDisplayCheck);
    document.addEventListener("visibilitychange", runDisplayCheck);

    return () => {
      cancelled = true;
      clearInterval(interval);
      window.removeEventListener("focus", runDisplayCheck);
      document.removeEventListener("visibilitychange", runDisplayCheck);
    };
  }, [examStarted, examFinished]);

  useEffect(() => {
    if (!isDisplayViolated || examFinished) return;

    const countdown = setInterval(() => {
      setDisplayGraceRemaining((prev) => {
        if (prev <= 1) {
          clearInterval(countdown);
          autoSubmitExam("Screen mirroring or extended display was not removed within 60 seconds.");
          return 0;
        }
  ...
  }, [isDisplayViolated, examFinished]);
...
useEffect(() => {
  ...
  const detectDisplayInterference = async () => {
    try {
      if (typeof window.getScreenDetails === "function") {
        ...
        if (screenCount > 1 || isExtended) {
          return {
            detected: true,
            message: `Display mirroring or an extended display is active (${screenCount} screens detected). Remove the extra display to continue.`,
          };
        }
      }
    } catch (error) {
      console.warn("Display detection is unavailable in this browser:", error);
    }

    if (window.screen?.isExtended === true) {
      return {
        detected: true,
        message: "An extended display is active. Remove it to continue the exam.",
      };
    }
  ...
  const clearDisplayViolation = () => {
    if (!isDisplayViolatedRef.current) return;

    isDisplayViolatedRef.current = false;
    setIsDisplayViolated(false);
    setDisplayGraceRemaining(DISPLAY_GRACE_PERIOD);
    setDisplayViolationMessage("");
    logEvent("Display configuration restored. Grace period cleared.");
  };

  const startDisplayViolation = (message) => {
    if (isDisplayViolatedRef.current) {
      if (message && message !== displayViolationMessageRef.current) {
        setDisplayViolationMessage(message);
      }
      return;
    }

    isDisplayViolatedRef.current = true;
    setIsDisplayViolated(true);
    setDisplayGraceRemaining(DISPLAY_GRACE_PERIOD);
    setDisplayViolationMessage(message);
    logEvent(`SECURITY ALERT: ${message} Grace period started: ${DISPLAY_GRACE_PERIOD} seconds.`);
  };

  const evaluateDisplaySetup = async () => {
    if (!examStartedRef.current || examFinishedRef.current || submittingRef.current) return;

    const result = await detectDisplayInterference();

    if (result.detected) {
      startDisplayViolation(result.message);
    } else {
      clearDisplayViolation();
    }
  };
  ...
  const sendExamData = async (videoBlob = null) => {
    ...
    try {
      ...
      formData.append("systemInfo", JSON.stringify({
        ...
        display: {
          isExtended: window.screen?.isExtended === true,
          displayViolationActive: isDisplayViolatedRef.current,
        },
...
const handleSelectOption = (option) => {
 ...
 const startNewExam = async () => {
    try {
      ...
      localStorage.removeItem("display_violation");
      ...
      isDisplayViolatedRef.current = false;
      displayViolationMessageRef.current = "";
...
if (!examStarted) {
  ...
  return (
    <div className="app-shell" style={{ position: "relative" }}>
      ...
      {isDisplayViolated && (
        <div className="app-banner app-banner--critical" style={{ marginTop: 16 }}>
          <strong>Display warning:</strong> {displayViolationMessage || "External display usage detected."} Remove it now. Auto-submit in {displayGraceRemaining} seconds.
        </div>
      )}
      ...
      <main className="app-main">
        ...
        <aside className="app-sidebar">
          ...
          <SecurityPanel
            ...
            displayViolation={isDisplayViolated}
            displayGraceRemaining={displayGraceRemaining}
            displayViolationMessage={displayViolationMessage}
          />
        </aside>
      </main>
    </div>
