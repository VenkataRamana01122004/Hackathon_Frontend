...
>
  {assessment.answers?.map((_, index) => (
    <button
      key={index}
      ...
      style={{
        ...
        fontWeight: 600,
        background:
          selectedProblem === index ? "#2563eb" : "#e5e7eb",
        color:
          selectedProblem === index ? "#fff" : "#000",
      }}
    >
      Problem {index + 1}
    </button>
...
{assessment.answers?.length ? (
  (() => {
    const answer = assessment.answers[selectedProblem];
